...
test('personal quiz saves each answer, restores measurements, edits and unlocks results', async ({
  ...
  await expect(page.getByText('YOUR PERSONAL PLAN', { exact: true }).first()).toBeVisible()
...
test('fixed mock route opens the paywall without completing the funnel', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Here’s your wellness profile' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Unlock your full summary' })).toBeVisible()
  await expect(page.getByText('YOUR PERSONAL PLAN', { exact: true }).first()).toBeVisible()
